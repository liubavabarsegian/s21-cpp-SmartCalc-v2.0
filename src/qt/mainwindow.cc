...
MainWindow::MainWindow(QWidget *parent)
  ...
  connect(ui->pushButton_credit, &QPushButton::clicked, this,
          [this] { ShowCreditView(); });
    ...
    if (ui->inputX->text() != "Incorrect!") {
      ...
      if (input.length() > 255) {
        ...
        if (!status) {
          ...
          ui->textEdit->setText(
              QString::fromLocal8Bit(calc_controller->GetResult().c_str()));
        }
      }
    }
  } else {
...
void MainWindow::ShowCreditView() { this->credit_view->show(); }
